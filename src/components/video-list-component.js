...
// loads all videos
  ...
  updateVideo(new_video_title, new_video_code, new_band, new_year, new_lyrics, new_genre, old_video_code, new_type, new_tags, new_stars) {
    new_lyrics = new_lyrics.replace(/[^A-Za-z0-9\s]/g,'');
    new_lyrics = new_lyrics.toLowerCase();
    new_lyrics = new_lyrics.trim();
    ...
    this.props.videos.update({video_code: old_video_code}, {$set:{video_code: new_video_code, video_title: new_video_title, video_band: new_band, video_year: new_year, video_lyrics: new_lyrics, video_genre: new_genre, video_type: new_type, video_tags: new_tags}}, function(err, doc) {
    });
    this.props.videos_shortterm.update({video_code: old_video_code}, {$set:{video_code: new_video_code, video_title: new_video_title, video_band: new_band, video_year: new_year, video_lyrics: new_lyrics, video_genre: new_genre, video_type: new_type, video_tags: new_tags}}, function(err, doc) {
    });
    ...
    // test for any matches
      ...
      // then set video_present to true
        video_present = true;
      }
    }
